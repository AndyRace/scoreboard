    ...
    public class FadeCandyUsbDevice
    {
            ...
            get
            {
                if (_singleton == null)
                {
                    lock (_singletonLock)
                    {
                        if (_singleton == null)
                        {
                            _singleton = new FadeCandyUsbDevice();
                        }
                    }
                }
        ...
        private void DoWriteData(byte[] data, int attempts = 0)
        {
            ...
            catch (Exception exception)
            {
                if (attempts < 1)
                {
                    _logger.Info($"FadeCandy.WriteData: Retrying... ({attempts})");
                    if (_usbDevice != null)
                    {
                        _usbDevice.Dispose();
                        _usbDevice = null;
        ...
        private void DebugPacket(byte[] data)
        {
            ...
            _logger.Info($"WriteData [{data.Length}]: [type={typeInfo}, final={finalBit}, index={packetIndex} ({packetIndex * 21})]: {info}");
        }
    }
}